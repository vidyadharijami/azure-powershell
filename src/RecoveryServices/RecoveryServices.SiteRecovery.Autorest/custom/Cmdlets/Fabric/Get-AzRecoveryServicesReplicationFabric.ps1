...
function Get-AzRecoveryServicesReplicationFabric {
    ...
    param(    
        [Parameter(ParameterSetName='GetViaName', HelpMessage='Friendly Name of the fabric')]
    ...
    process {
        try {
            $fabric = Az.RecoveryServices\Get-AzRecoveryServicesReplicationFabric -ResourceGroupName $ResourceGroupName -ResourceName $ResourceName | Where-Object { $_.FriendlyName -eq $FriendlyName }
            if ($fabric) {
                return $fabric
            }
            else{
                throw "ASR fabric with friendly name '$FriendlyName' not found."
            }
        } catch {
            throw
        }
